...
#        print "got message %r" % message
    ...
    def event_handler(self, message):
        ...
        if msg["event"] == "newMessage":
            ...
            #"send uhandled users "
            max_times = self.application.db.query("select a.customer_name\
            from in_data as a,\
            out_data as b where a.customer_name = b.customer_name \
            group by a.customer_name,\
            b.customer_name having max(a.insert_time) < max(b.insert_time) ")
            handled = set()
#            print max_times
#            for max_time in max_times:
#                if max_time.get("max(b.insert_time)") >
# max_time.get("max(a.insert_time)"):
...
#            print handled_string
            unhandled = self.application.db.query(
                "select distinct(customer_name)\
                from in_data where customer_name not in (%s)",
                handled_string[:-1])
            users = []
...
#            print "send uhandled"
            messages = self.application.db.query(
                "select content,content_type,create_time \
                from in_data where customer_name = %s",
                msg["username"])
            message = []
                ...
                message.append(
                    {
                        ...
                        "timestamp": time.mktime(
                            msgx.get("create_time").timetuple()
                            ),
...
#    prd = tornado.ioloop.PeriodicCallback(ChatSocketHandler.update_cache,
#    5000)
#    prd.start()
    loop.start()
